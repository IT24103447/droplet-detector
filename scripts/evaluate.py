...
def discover_fabrics(data_root: Path) -> list[str]:
    """Auto-detects fabric names from data/raw/<fabric>/ subfolders instead
    of a hardcoded list, so any fabric on hand works, not just the 5
    originally specified industrial types."""
...
def main() -> None:
    ...
    fabrics = discover_fabrics(data_root)
    
    if not fabrics:
        print(f"No fabric folders found under {data_root / 'raw'}")
        return
